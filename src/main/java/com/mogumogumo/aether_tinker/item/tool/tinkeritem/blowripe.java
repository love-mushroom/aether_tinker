package com.mogumogumo.aether_tinker.item.tool.tinkeritem;
    ...
    public InteractionResultHolder<ItemStack> world(Level level, Player player, InteractionHand hand) {
        ...
        if (!tool.isBroken() && toolData.getInt(this.BLOWPIPE_CHANCE) == 20) {
            ...
            if (!level.isClientSide()) {
                ...
                ArrowItem var10000;
                if (startAngle instanceof ArrowItem arrow) {
                    var10000 = arrow;
                } else {
            ...
            toolData.putInt(this.BLOWPIPE_CHANCE, (int)Math.min(20.0F, (double)toolData.getInt(this.BLOWPIPE_CHANCE) + (double)drawSpeed / 0.025));
        }
